...
public class One_IntroductionToMaps {
    ...
    //Map is an interface that extends Collection interface which is sued to store key-value type of information.
    //  Unique features:
    ...
    //  put(<k>, <v>)   - insert one pair of key and value
    //  putAll(<map>)   - insert all values of map into another
    //  remove(<k>)     - delete an entry form map, the key and values are deleted from map
    //  <v> get(<k>)    - returns the specific value inserted along with the key
    //  <bool> containsKey(<k>) - test to find a specific key in map
    //  keySet()        - returns Set of all keys from the map
    //  entrySet()      - returns the Set of view containing keys and values
